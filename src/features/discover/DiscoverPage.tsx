...
export function DiscoverPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 pb-44 pt-6 sm:px-8">
      {/* Ambient color wash for the "Liquid Glass" prototype (Discover-only,
          per the earlier scoped decision) — .glass-panel is a blur over
          whatever sits behind it, and a flat --background gave it nothing
          to actually refract. Fixed (not absolute) so it stays put while
          the page scrolls past it, and sits behind everything via -z-10. */}
      <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div className="absolute -left-24 -top-24 h-72 w-72 rounded-full bg-accent/25 blur-3xl dark:bg-accent/30" />
        <div className="absolute -right-20 top-40 h-80 w-80 rounded-full bg-sky-400/20 blur-3xl dark:bg-sky-500/25" />
        <div className="absolute bottom-0 left-1/3 h-96 w-96 rounded-full bg-amber-300/20 blur-3xl dark:bg-amber-500/20" />
      </div>
